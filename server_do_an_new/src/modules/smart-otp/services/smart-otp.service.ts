...
export class SmartOtpService {
  ...
  async registerDevice(
    ...
    return this.deviceBindingService.registerDevice(
      userId,
      publicKey,
      deviceFingerprint,
      ipAddress,
  ...
  async verifySmartOtp(
    ...
    try {
      ...
      const payload = this.signatureService.buildPayload(
        otp,
        timestamp,
        actionType,
      );
      ...
      if (!signatureValid) {
        ...
        const remainingAttempts =
          this.otpSessionService['MAX_ATTEMPTS'] - (session?.attempts || 0) - 1;
        return {
          valid: false,
      ...
      this.logger.log(
        `Verification Results: signatureValid=${signatureValid}, totpValid=${totpValid}`,
      );
