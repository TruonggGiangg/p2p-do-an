...
export class LoanService {
    ...
    async createApplication(
        ...
        const minRate = config.isAnnual ? (config.minInterestRatePerPeriod ?? 0) * 12 : (config.minInterestRatePerPeriod ?? 0);
        const maxRate = config.isAnnual ? (config.maxInterestRatePerPeriod ?? config.annualRate) * 12 : (config.maxInterestRatePerPeriod ?? config.annualRate / 12);
        ...
        if (minRate === maxRate && minRate > 0) {
            ...
            this.logger.log(`[createApplication] Rate clamped: ${rateForFineract} (isAnnual=${config.isAnnual}, min=${minRate} max=${maxRate})`);
        }
        ...
        this.logger.log(`[createApplication] Schedule: monthlyPay=${schedule.monthlyPay} entirelyPay=${schedule.entirelyPay}`);
        ...
        try {
            ...
            await doc.save();
            this.logger.log(`[createApplication] SUCCESS | mongoId=${doc._id} fineractLoanId=${fineractLoanId} status=pending`);
    ...
    async getApplicationHistory(userId: string): Promise<LoanHistoryItem[]> {
        ...
        if (fineractClientId) {
            ...
            for (const res of detailsResults) {
                ...
                const existing = Array.from(resultMap.values()).find(
                    (r) => r.fineractLoanId === loanId
                );
