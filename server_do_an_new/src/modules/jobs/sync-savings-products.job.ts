/**
 * SyncSavingsProductsJob — Đồng bộ sản phẩm tiết kiệm giữa Fineract ↔ MongoDB
 * Sử dụng AdminService.compareAndSyncSavings() để so sánh + lưu snapshot + ghi drift log
 */

import { BaseJob } from './base-job';
import { AdminService } from '../admin/admin.service';

export class SyncSavingsProductsJob extends BaseJob {
      ...
      paramsSchema: [
        {
          key: 'intervalMs',
          label: 'Tần suất chạy',
          description: 'Khoảng cách giữa mỗi lần đồng bộ sản phẩm tiết kiệm',
          type: 'interval',
        },
        {
          key: 'scheduleTime',
  ...
  async execute(): Promise<{
    ...
    this.logger.log('[execute] Comparing savings products with Fineract...');

    const result = await this.adminService.compareAndSyncSavings(true);
