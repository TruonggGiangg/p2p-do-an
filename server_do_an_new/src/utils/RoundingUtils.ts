 ...
 */
export const roundToCurrency = (amount: number, inMultiplesOf: number = 1000): number => {
 ...
 */
export const floorToCurrency = (amount: number, inMultiplesOf: number = 1000): number => {
  if (amount === undefined || amount === null || isNaN(amount)) {
    return 0;
  }
 ...
 * @param inMultiplesOf - Bội số làm tròn
 */
export const ceilToCurrency = (amount: number, inMultiplesOf: number = 1000): number => {
  if (amount === undefined || amount === null || isNaN(amount)) {
    return 0;
  }
 ...
 */
export const distributeEvenly = (total: number, parts: number, inMultiplesOf: number = 1000): number[] => {
  if (parts <= 0) return [];
 ...
 */
export const distributeByRatio = (total: number, ratios: number[], inMultiplesOf: number = 1000): number[] => {
