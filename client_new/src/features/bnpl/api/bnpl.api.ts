...
export const bnplAPI = {
    ...
    getWallet: async (): Promise<BnplWalletInfo> => {
        const response = await api.get<{ data: BnplWalletInfo }>('/api/bnpl/wallet');
        return response.data.data;
    },

    /** Preview BNPL loan (public) */
    previewLoan: async (data: { amount: number; numberOfRepayments?: number }): Promise<LoanPreview> => {
        const response = await api.post<{ data: LoanPreview }>('/api/bnpl/preview', data);
        return response.data.data;
    },

    /** Create new BNPL loan */
    createLoan: async (data: CreateBnplLoanDto): Promise<BnplLoan> => {
        const response = await api.post<{ data: BnplLoan }>('/api/bnpl/loans', data);
        return response.data.data;
    },
    ...
    getLoans: async (status?: string): Promise<LoansResponse> => {
        ...
        const response = await api.get<{ data: LoansResponse }>('/api/bnpl/loans', { params });
        return response.data.data;
    },

    /** Get loan details */
    getLoanDetails: async (loanId: string): Promise<BnplLoan> => {
        const response = await api.get<{ data: BnplLoan }>(`/api/bnpl/loans/${loanId}`);
        return response.data.data;
    },

    /** Sync loan status from Fineract */
    syncLoanStatus: async (loanId: string): Promise<BnplLoan> => {
        const response = await api.post<{ data: BnplLoan }>(`/api/bnpl/loans/${loanId}/sync`);
        return response.data.data;
    },

    /** Get consolidated repayment schedule */
    getConsolidatedSchedule: async (): Promise<ConsolidatedScheduleResponse> => {
        const response = await api.get<{ data: ConsolidatedScheduleResponse }>('/api/bnpl/schedule');
        return response.data.data;
    },
};
