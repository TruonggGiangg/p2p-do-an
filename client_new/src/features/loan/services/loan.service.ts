...
class LoanService {
    ...
    async getApplications(params?: {
        ...
        const response = await api.get<{ statusCode: number; data: LoanListResponse }>(`/api/loan/applications${qs}`);
        return response.data.data;
    }
     ...
     */
    async makeRepayment(loanId: string, amount: number, repaymentDate?: string): Promise<RepaymentResult> {
        const response = await api.post<{ statusCode: number; data: RepaymentResult }>('/api/loan/repay', {
            loanId,
            amount,
            repaymentDate,
        });
     ...
     */
    async prepayLoan(loanId: string, repaymentDate?: string): Promise<RepaymentResult> {
        const response = await api.post<{ statusCode: number; data: RepaymentResult }>('/api/loan/prepay', {
            loanId,
            repaymentDate,
        });
        return response.data.data;
    ...
    async getPrepayAmount(loanId: string): Promise<PrepayAmountResult> {
        const response = await api.get<{ statusCode: number; data: PrepayAmountResult }>(`/api/loan/${loanId}/prepay-amount`);
        return response.data.data;
    }
    ...
    async getOutstanding(loanId: string): Promise<OutstandingResult> {
        const response = await api.get<{ statusCode: number; data: OutstandingResult }>(`/api/loan/${loanId}/outstanding`);
        return response.data.data;
    }
    ...
    async getRepaymentSchedule(loanId: string): Promise<RepaymentScheduleResult> {
        const response = await api.get<{ statusCode: number; data: RepaymentScheduleResult }>(`/api/loan/${loanId}/schedule`);
        return response.data.data;
    }
    ...
    async getProductCharges(productId: number): Promise<ProductCharge[]> {
        const response = await api.get<{ statusCode: number; data: { charges: ProductCharge[] } }>(`/api/loan/products/${productId}/charges`);
