...
# Script: test-fineract-filter-raw.ps1
...
$resp5 = Invoke-WebRequest -Uri "$fineractBaseUrl/savingsaccounts?clientId=5" -Method Get -Headers $headers -UseBasicParsing
$resp5.Content

Write-Host "`n`n--- FETCHING FOR CLIENT 6 ---"
$resp6 = Invoke-WebRequest -Uri "$fineractBaseUrl/savingsaccounts?clientId=6" -Method Get -Headers $headers -UseBasicParsing
$resp6.Content
