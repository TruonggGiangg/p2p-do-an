### TEST CREATE LOAN WITH ML SCORECARD ###
...
# 2. Create loan with ML scorecard
$loanData = @{
    clientId = 1
    productId = 1
    principal = 20
...
Write-Host "`nRequest JSON:" -ForegroundColor Cyan
Write-Host $loanData
...
try {
    ...
    Write-Host "`nLoan created successfully!" -ForegroundColor Green
    Write-Host "Loan ID: $($response.loanId)" -ForegroundColor Cyan
} catch {
    Write-Host "`nError creating loan:" -ForegroundColor Red
